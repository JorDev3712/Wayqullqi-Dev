...
module.exports = {
    customId: 'OnUpdateWayVirtualSubmit',
    async execute(interaction, args) {
        ...
        const [resultCode, card] = await WayVirtualController.update(interaction, args[0], args[1], descriptionInput, Number(minSpendInput), Number(maxSpendInput), Number(noticeDayInput), noticeEnableInput.toLowerCase() === "si");
        if (resultCode == 0){
            await interaction.editReply({
                content: `🤖​ La Way Virtual "${card.description}" se guardó correctamente en su cuenta.`,
                flags: MessageFlags.Ephemeral
            });
        }
    },
